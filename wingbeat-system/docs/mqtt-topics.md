...
| `wingbeat/node/<id>/cmd/led` | browser → node | `{"mode":"solid","r":120,"g":40,"b":200,"intensity":0.8,"src":"router","seq":42,"ttlMs":3500}` | 1 | no | `mode` ∈ off · solid · pulse · shimmer · wind · rainbow. `src` identifies the publisher. `seq` rejects stale packets and `ttlMs` makes firmware return to off after publisher loss. `brightness` 0..1 optionally caps the strip. Feather/plant nodes only. |
| `wingbeat/node/<id>/cmd/audio` | browser → node | `{"layer":"accent","gain":0.8,"play":true,"seq":43,"ttlMs":5000}` | 1 | no | Audio-role nodes only. `layer` ∈ bed · melody · perc · accent; `loop` defaults to true for bed. Commanded playback returns to silence when its TTL expires. |
